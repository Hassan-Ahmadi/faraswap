...
class LoginView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user = User.objects.get(phone_number=request.data["phone_number"])  # Fetch user

        # Store user_id -> phone_number in Redis for `notif_service`
        cache.set(f"phone_number:{user.id}", user.phone_number, timeout=86400)  # Expire in 1 day
...
class VerifyOTPView(APIView):
    ...
    def post(self, request):
        ...
        if str(stored_otp) == str(otp_provided):
            cache.delete(f"otp:{phone_number}") # Delete OTP after successful verification
